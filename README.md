...
| [`tools/ownership-check.mjs`](tools/ownership-check.mjs) | [`common/rule-ownership.md`](common/rule-ownership.md) — no shared rule names a product, unless the name IS the instruction and says so in an `owns:` marker | `node tools/ownership-check.mjs [--warn] [--baseline <path>]` (this repository only; the per-file budget it ratchets down is [`tools/ownership-baseline.json`](tools/ownership-baseline.json), and deleting that file is the armed state) |
| [`tools/rule-bodies.mjs`](tools/rule-bodies.mjs) | The other half of the body freeze — it records the body of the rules you NAME, so an edit nobody named stays a red suite | `node tools/rule-bodies.mjs` to verify, `--update <id>…` to record (this repository only) |
