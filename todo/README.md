...
| [PLAN_dotnet_build_node_reuse.md](PLAN_dotnet_build_node_reuse.md) | `csharp/dotnet-build.md` — bound the MSBuild worker pool that concurrent agent builds leave behind: a `Directory.Build.rsp` per .NET consumer for `-nr:false`, `-m:N` on every build command, and the pin cascade |
