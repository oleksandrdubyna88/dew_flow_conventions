...
import { loadCatalog, normalizedText, selectRules, sha256, TASKS, targetPath } from "./lib/rule-catalog.mjs";
...
function validateInstructions(repo,files,selfHosted) {
  ...
  for(const file of files) {
    ...
    while(dir !== repo) {directories.add(dir);dir=path.dirname(dir);}
  }
  ...
  return [agents,claude,source(repo,selfHosted?"ENTRY.md":".agents/conventions/ENTRY.md"),source(repo,".agents/PROJECT.md")];
}
...
if(process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  catch(error) { console.error(`rules: INCOMPLETE — ${error.message}`); process.exitCode=1; }
}
